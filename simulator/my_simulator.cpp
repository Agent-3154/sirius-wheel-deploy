...
namespace {
    ...
    void InitializeSensor(mj::Simulate *sim) {
        mjv_defaultFigure(&sim->figsensor_bodygyro);
        mjv_defaultFigure(&sim->figsensor_bodyquat);
        mjv_defaultFigure(&sim->figsensor_bodyacc);
        //        mjvFigure &figsensor = sim->figsensor;
        mju::strcpy_arr(sim->figsensor_bodygyro.title, "body_gyro");
        mju::strcpy_arr(sim->figsensor_bodyquat.title, "body_rpy");
        mju::strcpy_arr(sim->figsensor_bodyacc.title, "body_acc");
        // x-label
        mju::strcpy_arr(sim->figsensor_bodygyro.xlabel, "Time");
        mju::strcpy_arr(sim->figsensor_bodyquat.xlabel, "Time");
        mju::strcpy_arr(sim->figsensor_bodyacc.xlabel, "Time");
        // y-label
        mju::strcpy_arr(sim->figsensor_bodygyro.yformat, "%0.2f");
        mju::strcmp_arr(sim->figsensor_bodyquat.yformat, "%0.2f");
        mju::strcmp_arr(sim->figsensor_bodyacc.yformat, "%0.2f");
        //background colors
        sim->figsensor_bodygyro.figurergba[0] = 0.1f;
        sim->figsensor_bodyquat.figurergba[2] = 0.2f;
        sim->figsensor_bodyacc.figurergba[0] = 0.1f;
        sim->figsensor_bodygyro.figurergba[3] = 0.5f;
        sim->figsensor_bodyquat.figurergba[3] = 0.5f;
        sim->figsensor_bodyacc.figurergba[3] = 0.5f;

        mju::strcpy_arr(sim->figsensor_bodygyro.linename[0], "wx");
        mju::strcpy_arr(sim->figsensor_bodygyro.linename[1], "wy");
        mju::strcpy_arr(sim->figsensor_bodygyro.linename[2], "wz");
        mju::strcpy_arr(sim->figsensor_bodyquat.linename[0], "roll");
        mju::strcpy_arr(sim->figsensor_bodyquat.linename[1], "pitch");
        mju::strcpy_arr(sim->figsensor_bodyquat.linename[2], "yaw");
        mju::strcpy_arr(sim->figsensor_bodyacc.linename[0], "ax");
        mju::strcpy_arr(sim->figsensor_bodyacc.linename[1], "ay");
        mju::strcpy_arr(sim->figsensor_bodyacc.linename[2], "az");

        // grid sizes
        // x y. two values
        sim->figsensor_bodygyro.gridsize[0] = 3;
        sim->figsensor_bodygyro.gridsize[1] = 5;

        sim->figsensor_bodyquat.gridsize[0] = 3;
        sim->figsensor_bodyquat.gridsize[1] = 5;
        sim->figsensor_bodyacc.gridsize[0] = 3;
        sim->figsensor_bodyacc.gridsize[1] = 5;

        // minimum ranges
        // range x y
        sim->figsensor_bodygyro.range[0][0] = -200;
        sim->figsensor_bodygyro.range[0][1] = 0;
        sim->figsensor_bodygyro.range[1][0] = -20;
        sim->figsensor_bodygyro.range[1][1] = 20;
        sim->figsensor_bodyquat.range[0][0] = -200;
        sim->figsensor_bodyquat.range[0][1] = 0;
        sim->figsensor_bodyquat.range[1][0] = -1;
        sim->figsensor_bodyquat.range[1][1] = 1;
        sim->figsensor_bodyacc.range[0][0] = -200;
        sim->figsensor_bodyacc.range[0][1] = 0;
        sim->figsensor_bodyacc.range[1][0] = -40;
        sim->figsensor_bodyacc.range[1][1] = 40;

        // init x axis on history figures (do not show yet)
        for (int n = 0; n < 3; n++) {
            for (int i = 0; i < mjMAXLINEPNT; i++) {
                sim->figsensor_bodygyro.linedata[n][2 * i] = -i;
                sim->figsensor_bodyquat.linedata[n][2 * i] = -i;
                sim->figsensor_bodyacc.linedata[n][2 * i] = -i;
            }
        }
    }

    // update sensor figure
    void UpdateSensor(mj::Simulate *sim, const mjModel *m, const mjData *d, bool real_sensor) {
        // gyro figure
        if (!real_sensor) {
            int pnt = mjMIN(201, sim->figsensor_bodygyro.linepnt[0] + 1);
            int adr_gyro = m->sensor_adr[0];
            int dim_gyro = m->sensor_dim[0];
            for (int n = 0; n < dim_gyro; n++) {
                // shift data
                for (int i = pnt - 1; i > 0; i--) {
                    sim->figsensor_bodygyro.linedata[n][2 * i + 1] = sim->figsensor_bodygyro.linedata[n][2 * i - 1];
                }
                sim->figsensor_bodygyro.linepnt[n] = pnt;
                sim->figsensor_bodygyro.linedata[n][1] = d->sensordata[adr_gyro + n];
                //            std::cout << "Sensor Data:[" << n << "]: " << d->sensordata[adr + n] << "\n";
            }
            // quat figure
            pnt = mjMIN(201, sim->figsensor_bodyquat.linepnt[0] + 1);
            int adr_quat = m->sensor_adr[1];
            int dim_quat = m->sensor_dim[1];
            mjtNum quat[4]; // w x y z
            mjtNum euler_angle[3];
            mjtNum rotation_matrix[9];
            for (int i = 0; i < dim_quat; i++) {
                quat[i] = d->sensordata[adr_quat + i];
            }
            mju_quat2Mat(rotation_matrix, quat);
            // ATTENTION: the matrix in mujoco is column dominated;
            euler_angle[0] = atan2(rotation_matrix[7], rotation_matrix[8]);
            euler_angle[1] = atan2(-rotation_matrix[6],
                                   sqrt(rotation_matrix[7] * rotation_matrix[7] +
                                        rotation_matrix[8] * rotation_matrix[8]));
            euler_angle[2] = atan2(rotation_matrix[3], rotation_matrix[0]);
            for (int n = 0; n < dim_quat - 1; n++) {
                // shift data
                for (int i = pnt - 1; i > 0; i--) {
                    sim->figsensor_bodyquat.linedata[n][2 * i + 1] = sim->figsensor_bodyquat.linedata[n][2 * i - 1];
                }
                sim->figsensor_bodyquat.linepnt[n] = pnt;
                sim->figsensor_bodyquat.linedata[n][1] = static_cast<float>(euler_angle[n]);
                //            std::cout << "Sensor Data:[" << n << "]: " << d->sensordata[adr + n] << "\n";
            }
            // acc figure
            pnt = mjMIN(201, sim->figsensor_bodyacc.linepnt[0] + 1);
            mjtNum cutoff = (m->sensor_cutoff[2] > 0 ? m->sensor_cutoff[2] : 1);
            int adr_acc = m->sensor_adr[2];
            int dim_acc = m->sensor_dim[2];
            for (int n = 0; n < dim_acc; n++) {
                // shift data
                for (int i = pnt - 1; i > 0; i--) {
                    sim->figsensor_bodyacc.linedata[n][2 * i + 1] = sim->figsensor_bodyacc.linedata[n][2 * i - 1];
                }
                sim->figsensor_bodyacc.linepnt[n] = pnt;
                sim->figsensor_bodyacc.linedata[n][1] = d->sensordata[adr_acc + n];
                //            std::cout << "Sensor Data:[" << n << "]: " << d->sensordata[adr + n] << "\n";
            }
        } else {
            int pnt = mjMIN(601, sim->figsensor_bodygyro.linepnt[0] + 1);
            int dim_gyro = m->sensor_dim[0];
            ...
            for (int n = 0; n < dim_gyro; n++) {
                ...
                sim->figsensor_bodygyro.linepnt[n] = pnt;
                sim->figsensor_bodygyro.linedata[n][1] = sim->sim_local_imudata.gyro[n];
                //            std::cout << "Sensor Data:[" << n << "]: " << d->sensordata[adr + n] << "\n";
            }
            // quat figure
            pnt = mjMIN(601, sim->figsensor_bodyquat.linepnt[0] + 1);
            int dim_quat = m->sensor_dim[1];
            mjtNum quat[4]; // w x y z
            mjtNum euler_angle[3];
            mjtNum rotation_matrix[9];
            for (int i = 0; i < dim_quat; i++) {
                quat[i] = sim->sim_local_imudata.q[i];
            }
            mju_quat2Mat(rotation_matrix, quat);
            // ATTENTION: the matrix in mujoco is column dominated;
            euler_angle[0] = atan2(rotation_matrix[7], rotation_matrix[8]);
            ...
            // acc figure
            pnt = mjMIN(601, sim->figsensor_bodyacc.linepnt[0] + 1);
            mjtNum cutoff = (m->sensor_cutoff[2] > 0 ? m->sensor_cutoff[2] : 1);
            int dim_acc = m->sensor_dim[2];
            for (int n = 0; n < dim_acc; n++) {
                // shift data
                ...
                sim->figsensor_bodyacc.linepnt[n] = pnt;
                sim->figsensor_bodyacc.linedata[n][1] = sim->sim_local_imudata.accel[n];
                //            std::cout << "Sensor Data:[" << n << "]: " << d->sensordata[adr + n] << "\n";{
            }
        }
    }
    ...
    void ShowSensor(mj::Simulate *sim, mjrRect rect) {
        ...
        mjr_figure(viewport, &sim->figsensor_bodygyro, &sim->platform_ui->mjr_context());
        viewport.bottom += rect.height / 3;
        mjr_figure(viewport, &sim->figsensor_bodyquat, &sim->platform_ui->mjr_context());
        viewport.bottom += rect.height / 3;
        mjr_figure(viewport, &sim->figsensor_bodyacc, &sim->platform_ui->mjr_context());
    }
...
#undef X
    ...
    void MakeJointSection(mj::Simulate *sim, int oldstate) {
        ...
        // for slider: state = 2 can be controlled; if state = 0, can not be interfaced
        defSlider[0].state = 0;
    ...
    void MakeControlSection(mj::Simulate *sim, int oldstate) {
        ...
        for (int i = 0; i < sim->actuator_ctrlrange_.size() && itemcnt < mjMAXUIITEM; i++) {
            ...
            //if use usb2can control cmd from lower state
            //            if (sim->use_usb2can_)
            defSlider[0].state = 2;
            // set data and name
            if (!sim->is_passive_) {
    ...
    int UiPredicate(int category, void *userdata) {
        auto *sim = static_cast<mj::Simulate *>(userdata);
    ...
    void UiLayout(mjuiState *state) {
        auto *sim = static_cast<mj::Simulate *>(state->userdata);
            ...
            switch (state->key) {
                ...
                case mjKEY_RIGHT: // step forward
                    if (!sim->is_passive_ && sim->m_ && !sim->run) {
                        ...
                        UpdateSensor(sim, sim->m_, sim->d_, sim->is_real_sensor_);
...
namespace mujoco {
    ...
    Simulate::Simulate(std::unique_ptr<PlatformUIAdapter> platform_ui, bool is_passive, bool is_real_sensor,
                       bool use_usb2can)
        : is_passive_(is_passive),
          is_real_sensor_(is_real_sensor),
          use_usb2can_(use_usb2can), platform_ui(std::move(platform_ui)), uistate(this->platform_ui->state()) {
        ...
        if (!is_real_sensor) {
            std::cout << BLUE << "[Sim Sensor]: " << RESET << "Loading Sim Sensor Panel!\n";
            mjuiDef local_def[13] = {
                ...
                {mjITEM_CHECKINT, "Sim_Sensor", 2, &this->sensor, " #293"},
            ...
            for (int i = 0; i < 13; i++) {
                def_option[i] = local_def[i];
            }
        } else {
            std::cout << BLUE << "[Real Sensor]: " << RESET << "Loading Real Sensor Panel!\n";
            mjuiDef local_def[13] = {
                {mjITEM_SECTION, "Option", 1, nullptr, "AO"},
                {mjITEM_CHECKINT, "Help", 2, &this->help, " #290"},
                {mjITEM_CHECKINT, "Info", 2, &this->info, " #291"},
                {mjITEM_CHECKINT, "Profiler", 2, &this->profiler, " #292"},
                {mjITEM_CHECKINT, "Real_Sensor", 2, &this->real_sensor, " #293"},
                {mjITEM_CHECKINT, "Pause update", 2, &this->pause_update, ""},
                {mjITEM_CHECKINT, "Fullscreen", 1, &this->fullscreen, " #294"},
                {mjITEM_CHECKINT, "Vertical Sync", 1, &this->vsync, ""},
                {mjITEM_CHECKINT, "Busy Wait", 1, &this->busywait, ""},
    ...
    void Simulate::Sync() {
        ...
        bool update_real_sensor = this->real_sensor && (this->pause_update || this->run);
...
#undef X
        ...
        if (update_sensor) { UpdateSensor(this, m_, d_, is_real_sensor_); }
        if (update_real_sensor) {
            std::lock_guard lk(sim_imu_mtx_);
            UpdateSensor(this, m_, d_, is_real_sensor_);
        }
    ...
    void Simulate::LoadMessage(std::string &displayed_filename) {
        //        mju::strcpy_arr(this->filename, displayed_filename.c_str());
        this->filename = displayed_filename; {
            MutexLock lock(mtx);
            this->loadrequest = 3;
        }
    }

    void Simulate::Load(mjModel *m, mjData *d, std::string &displayed_filename) {
        this->mnew_ = m;
        this->dnew_ = d;
        this->filename = displayed_filename;
        //        mju::strcpy_arr(this->filename, displayed_filename.c_str());
    ...
    void Simulate::LoadOnRenderThread() {
        ...
        if (this->filename[0] && this->filename.compare(this->previous_filename)) {
    ...
    void Simulate::RenderLoop() {
        ...
        mjui_add(&this->ui0, this->def_usr);
        ...
        // run event loop
        // NOTE: This loop consumes about 16800us, so this thread is only for visualization.
        // Due to the slow render, the control loop would be stuck by the render loop.
        // The cause is the interface with slider!!!!!
        while (!this->platform_ui->ShouldCloseWindow() && !this->exitrequest.load()) {
            {
                const MutexLock lock(this->mtx);
                // load model (not on first pass, to show "loading" label)
                // if load model, all data is initialized
                ...
                // used for uploading mesh, no use
                ...
                if (!this->is_passive_) {
                    // if (use_usb2can_) usb_2_can_LCM_.handleTimeout(0);
                    Sync();
                } else {
    ...
    void Simulate::set_foot_trajectory(const mjtNum from[3], const mjtNum to[3], int leg_id) {
        for (int i = 0; i < 3; i++) {
            user_geoms.from_foot_[leg_id][3 * geom_iter_[leg_id] + i] = from[i];
            user_geoms.to_foot_[leg_id][3 * geom_iter_[leg_id] + i] = to[i];
        }
        user_geoms.active_geoms_[leg_id]++;
        if (user_geoms.active_geoms_[leg_id] > Config::drawing_geom_number_feets) {
            user_geoms.active_geoms_[leg_id] = Config::drawing_geom_number_feets;
        }
        geom_iter_[leg_id]++;
        if (geom_iter_[leg_id] == Config::drawing_geom_number_feets) {
            geom_iter_[leg_id] = 0;
        }
    }

    void Simulate::set_pos_trajectory_des(const mjtNum from[3], const mjtNum to[3]) {
        for (int i = 0; i < 3; i++) {
            user_geoms.from_pos_d_[3 * geom_iter_pos_des + i] = from[i];
            user_geoms.to_pos_d_[3 * geom_iter_pos_des + i] = to[i];
        }
        user_geoms.active_geoms_pos_des_++;
        if (user_geoms.active_geoms_pos_des_ > Config::drawing_geom_number_pw) {
            user_geoms.active_geoms_pos_des_ = Config::drawing_geom_number_pw;
        }
        geom_iter_pos_des++;
        if (geom_iter_pos_des == Config::drawing_geom_number_pw) {
            geom_iter_pos_des = 0;
        }
    }

    void Simulate::set_pos_trajectory(const mjtNum from[3], const mjtNum to[3]) {
        for (int i = 0; i < 3; i++) {
            user_geoms.from_pos_[3 * geom_iter_pos_ + i] = from[i];
            user_geoms.to_pos_[3 * geom_iter_pos_ + i] = to[i];
        }
        user_geoms.active_geoms_pos_++;
        if (user_geoms.active_geoms_pos_ > Config::drawing_geom_number_pw) {
            user_geoms.active_geoms_pos_ = Config::drawing_geom_number_pw;
        }
        geom_iter_pos_++;
        if (geom_iter_pos_ == Config::drawing_geom_number_pw) {
            geom_iter_pos_ = 0;
        }
    }
    ...
    void Simulate::set_foot_trajectory_des(const mjtNum from[3], const mjtNum to[3], int leg_id) {
        for (int i = 0; i < 3; i++) {
            user_geoms.from_foot_d_[leg_id][3 * geom_iter_des_[leg_id] + i] = from[i];
            user_geoms.to_foot_d_[leg_id][3 * geom_iter_des_[leg_id] + i] = to[i];
        }
        user_geoms.active_geoms_des_[leg_id]++;
        if (user_geoms.active_geoms_des_[leg_id] > Config::drawing_geom_number_feets) {
            user_geoms.active_geoms_des_[leg_id] = Config::drawing_geom_number_feets;
        }
        geom_iter_des_[leg_id]++;
        if (geom_iter_des_[leg_id] == Config::drawing_geom_number_feets) {
            geom_iter_des_[leg_id] = 0;
        }
    }
    ...
    void Simulate::draw_user_geoms() {
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < Config::drawing_geom_number_feets; j++) {
                ...
                const mjtNum *to_temp = &user_geoms.to_foot_d_[i][3 * j];
                mjv_connector(&user_geoms.geom_foot_d_[i][j], mjGEOM_LINE, 2, from_temp, to_temp);
            }

            for (int j = 0; j < Config::drawing_geom_number_feets; j++) {
                const mjtNum *from_temp = &user_geoms.from_foot_[i][3 * j];
                const mjtNum *to_temp = &user_geoms.to_foot_[i][3 * j];
                mjv_connector(&user_geoms.geom_foot_[i][j], mjGEOM_LINE, 2, from_temp, to_temp);
            }

            std::memcpy(scn.geoms + scn.ngeom,
                        &user_geoms.geom_foot_d_[i],
