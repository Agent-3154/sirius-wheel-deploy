...
HardwareBridge::My_HardwareBridge::setup_HardwareBridge(const bool real_imu, const bool real_usb2can,
    ...
    // tp_usb_ = std::make_shared<Utilities::ThreadPool>(1);

    t_fdsdk_hardwares_ = std::make_shared<Thread::thread_fdsdk_hardwares>("Free Dog SDK", 500);
    // tp_fdsk_ = std::make_shared<Utilities::ThreadPool>(1);
    ...
    if (!unitree_) {
        ...
        if (real_usb2can) {
            usb2can_board_handle_ = new USB_HARDWARE::Motor_Control_Board(Config::usb2can_vendor_id,
        ...
        // use unitree hardwares
        my_fdsc_ = new My_FDSC(low_wired);
        my_fdsc_->set_buffer(usb_cmd_, usb_data_, usb_imu_);
        robot_runner_->runner_fdsc_ = my_fdsc_;
        // tp_fdsk_->Schedule([this] { t_fdsdk_hardwares_->thread_loop(my_fdsc_); });
        thread_free_sdk_ = std::thread(&My_HardwareBridge::thread_free_sdk_function, this);
    }
...
void HardwareBridge::My_HardwareBridge::thread_free_sdk_function() {
    my_fdsc_->FDSC_init();
    std::cout << GREEN << "[Thread FDSDK OK]: " << RESET << "Initialize free dog SDK thread!\n";
    while (true) {
        t_fdsdk_hardwares_->thread_enter_task();
        my_fdsc_->parse_data();
        my_fdsc_->pack_cmd();
        t_fdsdk_hardwares_->thread_finish_task();
    }
}
