...
namespace FDSC {
    ...
    public:
        ...
        std::vector<MotorState> motorState = std::vector<MotorState>(
            20, MotorState(motormode, q, dq, ddq, tauEst, q_raw, dq_raw, ddq_raw, motor_temperature, res_bms)); //
        uint8_t version_h = 0;
        uint8_t version_l = 0;
        ...
        BmsState bm_s = BmsState(version_h, version_l, bms_status, SOC, current, cycle, BQ_NTC, MCU_NTC, cell_vol);
        // 835-869
        ...
        std::vector<Cartesian> footPosition2Body = std::vector<Cartesian>(4, Cartesian(0.0, 0.0, 0.0));
        //48 bytes 943-991
