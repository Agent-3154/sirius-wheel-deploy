...
namespace FDSC {
    ...
    public:
        ...
        BmsState(uint8_t vh, uint8_t vl, uint8_t bs, uint8_t soc, int32_t curr, uint16_t cyc, std::vector<uint8_t> &bq,
                 std::vector<uint8_t> mcu, std::vector<uint16_t> &cv)
            : version_h(vh), version_l(vl), bms_status(bs), SOC(soc), current(curr), cycle(cyc), BQ_NTC(bq),
              MCU_NTC(mcu), cell_vol(cv) {
        }
    };
    ...
    public:
        ...
        MotorState(int m, float q_, float dq_, float ddq_, float tau, float qr, float dqr, float ddqr, float temp,
                   const std::vector<uint8_t> &res)
            : mode(m), q(q_), dq(dq_), ddq(ddq_), tauEst(tau), q_raw(qr), dq_raw(dqr), ddq_raw(ddqr), temperature(temp),
              reserve(res) {
        }

        inline void set_data(int m, float q_, float dq_, float ddq_, float tau, float qr, float dqr, float ddqr,
                             float temp, const std::vector<uint8_t> &res) {
            mode = m;
            q = q_;
            dq = dq_;
            ddq = ddq_;
    ...
    public:
        ...
        Imu(const std::vector<float> &quat, const std::vector<float> &gyro, const std::vector<float> &accel,
            const std::vector<float> &r, uint8_t temp)
    ...
    public:
        ...
        inline void set_data(const MotorModeLow m, const float q_, const float dq_, const float tau_, const float kp_,
                             const float kd_) {
            mode = m;
            q = q_;
            dq = dq_;
    ...
    public:
        ...
        inline void setMotorCmd(const std::string &motorIndex, const MotorModeLow m_,
                                const std::vector<float> &jointdata) {
            int index;
